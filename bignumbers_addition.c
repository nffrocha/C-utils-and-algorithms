/* By Nelson Rocha <nffrocha@gmail.com> 
The program crashes on some situations, i think that's a realloc() problem, i tried to figured it out but realloc() always returns
not NULL altough the problem exist there:\ If you find the bug please warn me
coolmaster@ptlink.net

This program adds two numbers of any kind of length, since each digit is stored in a char* the memory is the limit, it uses stacks
